...
createMailingAddress = "CREATE TABLE IF NOT EXISTS mailingAddress (
  first_line VARCHAR,
  second_line VARCHAR,
  city_name VARCHAR,
  state_name VARCHAR,
...
insertMailingAddress = "INSERT INTO mailingAddress VALUES ("

createPracticeAddress = "CREATE TABLE IF NOT EXISTS practiceAddress (
  first_line VARCHAR,
  second_line VARCHAR,
  city_name VARCHAR,
  state_name VARCHAR,
...
insertPracticeAddress = "INSERT INTO practiceAddress VALUES ("
...
def nullEmptyCheck(val)
  val.delete!('"')
  val.gsub!("'", "''")
  output = (val.empty?) ? "DEFAULT" : "'"+val+"'"
  return output
end

def createInsert(csvIndicies, line, nppesCsvLayout, reportNull = false)
  output = ""
  nonNullCount = 0
  csvIndicies.each_with_index do |indicie, index|
    ending = index == csvIndicies.length-1 ? '' : ','
    deafultCheck = nullEmptyCheck(line[nppesCsvLayout[indicie]])
...
conn.exec("DROP TABLE IF EXISTS mailingAddress")
conn.exec("DROP TABLE IF EXISTS practiceAddress")
...
conn.exec(createMailingAddress)
conn.exec(createPracticeAddress)
...
File.open(ARGV[1], 'r').each_line do |line| 
  ...
  i = i + 1
  splitLine = line.force_encoding("utf-8").split('","')
  ...
  (
    ...
    :authorized_official_credential], splitLine, nppesCsvLayout) + ');'
  )+
  insertMailingAddress+
  (
    ...
    :provider_business_mailing_address_telephone_number,:provider_business_mailing_address_fax_number,:npi], splitLine, nppesCsvLayout) + ');'
  )+
  insertPracticeAddress+
  (
    ...
    :provider_business_practice_location_address_telephone_number,:provider_business_practice_location_address_fax_number,:npi], splitLine, nppesCsvLayout) + ');'
  )
  for j in 1..15
    toInsert = createInsert(["healthcare_provider_taxonomy_group_#{j}".to_sym,:npi], splitLine, nppesCsvLayout, true) 
    if toInsert == nil
      next
    end
  ...
  for j in 1..50
    toInsert = createInsert(["other_provider_identifier_#{j}".to_sym, "other_provider_identifier_type_code_#{j}".to_sym, 
      "other_provider_identifier_state_#{j}".to_sym, "other_provider_identifier_issuer_#{j}".to_sym,:npi], splitLine, nppesCsvLayout, true) 
    if toInsert == nil
      next
    end
  ...
  for j in 1..15
    toInsert = createInsert(["healthcare_provider_taxonomy_code_#{j}".to_sym, "provider_license_number_#{j}".to_sym, 
      "provider_license_number_state_code_#{j}".to_sym, "healthcare_provider_primary_taxonomy_switch_#{j}".to_sym,:npi], splitLine, nppesCsvLayout, true) 
    if toInsert == nil
      next
    end
    output = output + insertOtherProvider + toInsert + ');'
