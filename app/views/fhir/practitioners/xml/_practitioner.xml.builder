xml.Practitioner(xmlns:"http://hl7.org/fhir") do
  ...
  [{ system: "phone", number: provider.mailing_address.telephone_number, period: nil }, 
    ...
    xml.telecom do
      xml << render(partial: 'fhir/practitioners/xml/contact.xml.builder', locals: locals) 
    end unless locals[:number].blank?
  end

  xml.address do
    xml << render(partial: 'fhir/practitioners/xml/address.xml.builder', locals: {address: provider.practice_location_address})
  end

  xml.gender do
    xml << render(partial: 'fhir/practitioners/xml/codeable_concept.xml.builder', 
      locals: { codings: [{code: provider.gender_code, display: provider.gender_code}], text: nil})
  end

  xml.organization do
    xml << render(partial: 'fhir/practitioners/xml/resource.xml.builder', 
  ...
  xml.period do
    if provider.npi_reactivation_date
      xml << render(partial: 'fhir/practitioners/xml/period.xml.builder', 
